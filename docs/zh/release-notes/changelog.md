...
- Skill：支持通过 `kimi marketplace install <plugin>@<marketplace>` 从 marketplace 安装社区 skill——安装后的 skill 名会加上插件前缀以避免冲突

- Skill：新增 `command` 类型 skill，通过 `/command:<name>` 作为一次性 user prompt 运行——marketplace 插件命令会被安装到 `~/.kimi/commands/`，并像斜杠命令一样调用

- MCP：自动加载 marketplace 插件贡献的 MCP server 配置，从 `~/.kimi/plugin-mcp.json` 读取——server 名会加上插件前缀以防止冲突
